...
<body>
	...
	<table class="table table-responsive bg-info">
		<tr style="background: rgb(92, 92, 94);text-align:center;">
			...
			<td style="color:#fff; font-weight:bold; padding:10px;">Total Present</td>
			<td style="color:#fff; font-weight:bold; padding:10px;">Total Class</td>
		...
		<tr class="{{ $isEven ? 'even-row' : 'odd-row' }}" style="text-align:center;">
			...
			<td>{{ $student->my_attendence_count / ($totalAttendence == 0 ? 1 : $totalAttendence) * 100 ."%" }}</td>
		</tr>
		@endforeach
		@endif
